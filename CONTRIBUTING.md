...
- **`fast`** — `InTest.Architecture.Tests`, `InTest.Cli.Tests` and `InTest.Runtime.Tests`, each
  ...
  platform: ~33.5–35.5s end to end — that figure predates the xUnit adapter pack and should be
  treated as stale (CLAUDE.md records a similar climb for the `golden` figure below; re-measure
  rather than trust it). **This job's composition changed once the xUnit adapter pack landed.**
  The two adapters (`InTest.Runtime.MSTest`, `InTest.Runtime.xUnit`) declare the same types in the
  same namespace, so they cannot share a compilation — `InTest.Runtime.Tests` still exercises only
  the MSTest adapter (an `InternalsVisibleTo` grant from it), and the xUnit adapter's own suite,
  `InTest.Runtime.XUnit.Tests`, was added to this job as a **separate direct-executable step**,
  not another `dotnet test` line — a solution-level `dotnet test InTest.sln` fails outright once
  both frameworks' projects exist in one solution (the xUnit project errors on the VSTest target
  while the MSTest ones run and pass).
...
Five packages now ship together and their majors move together: `InTest.Cli`, `InTest.Runtime`
(the framework-neutral fixture/schema/identity runtime — no test-framework dependency),
...
`docs/superpowers/plans/2026-08-25-runtime-framework-split.md` is the record of that split),
`InTest.Runtime.xUnit` (the xUnit v3 adapter — the same `TestHost`/`ApiTestBase` shape, in the
same `namespace InTest.Runtime`, depending on `InTest.Runtime` at the exact same version plus
`xunit.v3.extensibility.core` and `xunit.v3.assert`;
`docs/superpowers/specs/2026-08-30-intest-xunit-framework-pack.md` is the record of that
addition), and `InTest.Runtime.NUnit` (the NUnit adapter — the same `TestHost`/`ApiTestBase`
shape again, depending on `InTest.Runtime` at the exact same version plus `NUnit`, with no
`extensibility.core`/`assert` split since `NUnit` alone compiles an ordinary class library;
`docs/superpowers/plans/2026-08-31-intest-nunit-framework-pack.md` is the record of that
addition). A generated project references whichever adapter matches its `project.framework` and
gets `InTest.Runtime` transitively. The compatibility contract is in §3 of the spec and is public
API:

- `InTest.Runtime`/`InTest.Runtime.MSTest`/`InTest.Runtime.xUnit`/`InTest.Runtime.NUnit` **N.x**
  accept code generated by `InTest.Cli` **N.y** for any `y`.
...
3. Tag that commit and push the tag: `git tag 0.1.0-preview.1 && git push origin 0.1.0-preview.1`
   ...
   and does not accept.
4. `pack.yml` packs and verifies all five packages at exactly that tag's version and uploads them
...
5. `release.yml` runs in parallel (same tag push triggers both workflows independently): its own
   ...
   all five packages' `.nupkg`/`.snupkg` attached (ten assets — `release.yml` itself asserts the
   count, derived from a package-id list rather than a literal number). It runs after `publish`,
   in `contents: write` only, never in the same job as the
...
conclusion. **Also unproven by this run:** the `InTest.Runtime`/`InTest.Runtime.MSTest` split, and
the later `InTest.Runtime.xUnit` and `InTest.Runtime.NUnit` additions, all landed after it, so no
...
8. **Verify, then tag.** Merge the release candidate to `main` and let `.github/workflows/pack.yml`
   ...
   *before* an irreversible push under an otherwise-automated flow, using an artifact `pack.yml`
   ...
   job mid-run.
9. `release.yml` packs and pushes each package's `.nupkg` and `.snupkg` automatically — five
   packages, ten files, no manual `dotnet nuget push` needed or expected. Confirm the run went
   green and every version now appears on nuget.org. **Confirmed by the `0.1.0-preview.1` push**
   (`InTest.Cli` and `InTest.Runtime` only — before the `InTest.Runtime.MSTest` split and the later
...
hand.** The script already packs `InTest.Cli`, `InTest.Runtime`, `InTest.Runtime.MSTest`,
`InTest.Runtime.xUnit` and `InTest.Runtime.NUnit`, and the scaffolded project it builds references
whichever adapter package matches `-Framework` (`InTest.Runtime.MSTest`, `InTest.Runtime.xUnit` or
`InTest.Runtime.NUnit`), matching what `intest init` actually writes.

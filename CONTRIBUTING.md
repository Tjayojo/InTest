...
anything past that tag. A third and fourth package — `InTest.Runtime.MSTest` and
`InTest.Runtime.xUnit`, the MSTest and xUnit adapters split out of `InTest.Runtime`, described in
"Releases" below — have not been published at all yet, so `examples/` still pins the neutral
`InTest.Runtime` package directly at `0.1.0-preview.1` rather than either adapter it would
reference once one exists on nuget.org. The
...
Four packages now ship together and their majors move together: `InTest.Cli`, `InTest.Runtime`
...
`docs/superpowers/plans/2026-08-25-runtime-framework-split.md` is the record of that split), and
...
- `InTest.Runtime`/`InTest.Runtime.MSTest`/`InTest.Runtime.xUnit` **N.x** accept code generated by
  `InTest.Cli` **N.y** for any `y`.
...
3. Tag that commit and push the tag: `git tag 0.1.0-preview.1 && git push origin 0.1.0-preview.1`
   ...
   and does not accept.
4. `pack.yml` packs and verifies all four packages at exactly that tag's version and uploads them
...
5. `release.yml` runs in parallel (same tag push triggers both workflows independently): its own
   ...
   all four packages' `.nupkg`/`.snupkg` attached (eight assets — `release.yml` itself asserts the
...
the later `InTest.Runtime.xUnit` addition, both landed after it, so no tag push has yet published
the four-package, eight-artifact shape described above — the next tag push is the first real test
of that.
...
8. **Verify, then tag.** Merge the release candidate to `main` and let `.github/workflows/pack.yml`
   ...
   *before* an irreversible push under an otherwise-automated flow, using an artifact `pack.yml`
   ...
   job mid-run.
9. `release.yml` packs and pushes each package's `.nupkg` and `.snupkg` automatically — four
   packages, eight files, no manual `dotnet nuget push` needed or expected. Confirm the run went
   ...
   see `docs/v0-acceptance.md`. The next tag push is the first to exercise the four-package,
   eight-file shape.
...
11. Starting with the release *after* each package's first publish: add
    ...
    `InTest.Runtime.MSTest` and `InTest.Runtime.xUnit` too. (`InTest.Cli` never participates in
    package validation — the SDK hard-disables it for tool packages.)

...
public class GenerateCheckCommandTests
{
    ...
    /// now names both.
    ///
    /// The two loose <c>ShouldContain</c> checks this test used to run passed on Linux for the
    /// wrong reason (found running the InTest CI plan's Task 1 on Linux for the first time,
    /// 2026-08-22): <see cref="File.Exists"/> is case-*sensitive* there, so the content-compare
    /// loop's own <c>File.Exists</c> check on the expected path "Generated/OrdersTests.g.cs" is
    /// false, adding an extra "Generated/OrdersTests.g.cs is missing." line the orphan sweep's
    /// "case differs" line still followed — a genuinely different code path (two lines, not one)
    /// that the substring checks below could not tell apart from the single-line Windows case.
    /// Asserting the exact ordered line set makes the two platforms' outputs distinguishable and
    /// pins both deliberately.
    /// </summary>
    [TestMethod]
    public async Task NamesTheExpectedCasingWhenOnlyCasingDiffersFromAFreshRender()
    {
        ...
        const string caseDiffersLine =
            "Generated/orderstests.g.cs exists on disk but a fresh render names it " +
            "Generated/OrdersTests.g.cs instead (case differs).";

        // On Windows, File.Exists("Generated/OrdersTests.g.cs") matches the renamed
        // "orderstests.g.cs" case-insensitively, so the content-compare loop finds it and (bytes
        // identical) emits nothing; only the orphan sweep's case-differs line fires. On Linux,
        // File.Exists is case-sensitive, so the same lookup is false and the content-compare loop
        // adds "is missing" first — the orphan sweep still runs afterward and adds its line too,
        // so Linux sees both, sorted ordinally ('O' < 'o' puts "is missing" first).

...
typed-client path captures responses differently. Two idioms have diverged; that divergence is the
real cost, and the line count is its symptom.
...
**It puts the stack trace in the runtime.** When a chain link throws, the failure points into
`InTest.Runtime` rather than the generated test, which is what principle #6 exists to prevent.
...
### `[captured-is-the-single-shape]` — one result type, four assertions become two
...
**P1 — the `CancellationToken` seam.** `Operation`/`ExpectStatus` moves the send into
`ApiTestCore`, which is neutral and **may never name `TestContext`** — `NeutralityTests` enforces
that at source level. So the token needs the same treatment as the four seams already extracted
(`IRunDiagnostics`, the profile string, the display name, the skip reason): a `CancellationToken`
parameter on `ApiTestCore.BeginTest`, supplied by `ApiTestBase.ApiTestInitialize`. Without it a
cancelled run keeps issuing HTTP, and it interacts with the template's load-bearing
`ex is not OperationCanceledException` filter.
...
Budget for Golden: ~90s warm locally, materially longer from a fresh worktree. Pass an explicit
timeout.
...
- `src/InTest.Cli/Rendering/Templates/mstest-class.scriban` and `TemplateRenderer.cs` — emit the
  consolidated call; **role gating unchanged**.
